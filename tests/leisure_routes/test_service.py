from unittest.mock import patch

from geoalchemy2.shape import from_shape
from shapely.geometry import MultiLineString

from app.leisure_routes.models import LeisureRoute
from app.leisure_routes.service import get_leisure_route, list_leisure_routes


def _fake_leisure_route(leisure_route_id: int = 1) -> LeisureRoute:
    return LeisureRoute(
        id=leisure_route_id,
        source_id="123",
        name="Rota Teste",
        support_count=5,
        geometry=from_shape(
            MultiLineString([[(-38.5, -3.7), (-38.51, -3.71)]]), srid=4326
        ),
    )


def test_list_leisure_routes_builds_correct_pagination_meta():
    with patch(
        "app.leisure_routes.service.list_paginated",
        return_value=([_fake_leisure_route()], 101),
    ):
        result = list_leisure_routes(session=None, page=2, page_size=50)
    assert result.meta.page == 2
    assert result.meta.page_size == 50
    assert result.meta.total == 101
    assert result.meta.total_pages == 3


def test_list_leisure_routes_pagination_meta_zero_total():
    with patch("app.leisure_routes.service.list_paginated", return_value=([], 0)):
        result = list_leisure_routes(session=None, page=1, page_size=50)
    assert result.meta.total == 0
    assert result.meta.total_pages == 0
    assert result.features == []


def test_list_leisure_routes_converts_rows_to_valid_features():
    with patch(
        "app.leisure_routes.service.list_paginated",
        return_value=([_fake_leisure_route(7)], 1),
    ):
        result = list_leisure_routes(session=None, page=1, page_size=50)
    feature = result.features[0]
    assert feature.type == "Feature"
    assert feature.geometry["type"] == "MultiLineString"
    assert feature.properties.id == 7


def test_get_leisure_route_returns_none_when_repository_returns_none():
    with patch("app.leisure_routes.service.get_by_id", return_value=None):
        assert get_leisure_route(session=None, leisure_route_id=999) is None


def test_get_leisure_route_returns_feature_for_existing_row():
    with patch("app.leisure_routes.service.get_by_id", return_value=_fake_leisure_route(5)):
        feature = get_leisure_route(session=None, leisure_route_id=5)
    assert feature is not None
    assert feature.properties.id == 5
